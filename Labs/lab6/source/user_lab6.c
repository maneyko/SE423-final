	...
	if ((timecount % 1000) == 0) {
		...
		switch (ss) {
            case 0:  // Print LADAR distances
                LCDPrintfLine(1, "LD[1]:%.3f,LD[225]:%.3f,LD[172]:%.3f",
                              LADARdistance[1], LADARdistance[225], LADARdistance[172]);
                LCDPrintfLine(2, "LD[54]:%.3f,LD[113]:%.3f",
                              LADARdistance[54], LADARdistance[113]);
                break;
            case 1:  // ADC IR distances
                LCDPrintfLine(1, "IR0:%.3f,IR1:%.3f",
                              adcA4, adcA5);
                LCDPrintfLine(2, "IR2:%.3f,IR3:%.3f", adcA6, adcA7);
                break;
            case 2:
                LCDPrintfLine(1, "Compass:%.3f", compass);
                LCDPrintfLine(2, "2g");
                break;

            case 3:
                LCDPrintfLine(1, "Gyro1:%.3f", adcA3 * 360 / (2*3.14159));
                LCDPrintfLine(2, "Gyro2:%.3f", adcA2 * 360 / (2*3.14159));
                break;

            case 4:
                LCDPrintfLine(1, "Ultra1:%.3f", adcB4);
                LCDPrintfLine(2, "Ultra2:%.3f", adcB5);
                break;

            case 5:
                LCDPrintfLine(1, "Vref:%.3f)", vref);
                LCDPrintfLine(2, "turn:%.3f)", turn);
                break;
