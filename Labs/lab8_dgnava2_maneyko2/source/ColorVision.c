...
void userProcessColorImageFunc_laser(bgr *ptrImage) {  // runs every 40ms
    ...
    if (ptrImage != NULL) {
        ...
        //      blue_x_obj = rbar;
        //        blue_y_obj = cbar;
        //        Nblue = largest_num_pixels;

        // pass data to RobotControl()
        if (new_coordata == 0) {
            if (final_object_stats[largest_object].num_pixels_in_object > 5) {
                noimagefound = 0;
