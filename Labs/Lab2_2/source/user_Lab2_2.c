...
void SWI_function(void) {  // Handle: SWI_counttime
    counter1++;
    if (counter1 % 100 == 0) {
        LCDPrintfLine(1, "time lost %d", counter1);
        LCDPrintfLine(2, "time lost %.3f", (float)counter1/1000.0);
    }
    if ((counter1 % 20) == 0) {
        Swi_post(SWI_loaded);
    }
...
void CLK_every1ms(void) {
    Swi_post(SWI_counttime);
    //Swi_post(SWI_function);
}
...
void stallfunc(void) {  // Handle: SWI_loaded
    timeload(10);
}
