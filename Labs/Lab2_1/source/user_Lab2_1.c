...
void CLK_1(void) {
    countvar++;
    if (countvar % 250 == 0) {
        LCDPrintfLine(2, "The count is %d", countvar);
        // LCDPrintfLine(1, "Hey, I made a program.");
    }
}
...
void task2_1(void) {
    while(1) {
        ...
        task2_counter++;
        LCDPrintfLine(1, "Task2 called %d", task2_counter);
        // Semaphore_pend(semaphore0, );
    }
}
...
void ComWithLinux(void) {
	...
	while(1) {
		...
		if (GET_LVDATA_FROM_LINUX == 1) {  // see of Linux put data in shared memory
		    // read shared memory
	        Cache_inv((void *)ptrshrdmem,sizeof(sharedmemstruct), Cache_Type_ALL, EDMA3_CACHE_WAIT);

	        Semaphore_post(semaphore0);
	        int j;
	        for (j = 0; j < ptrshrdmem->DSPRec_size; j++) {
	            mystring[j] = ptrshrdmem->DSPRec_buf[j];
	        }
	        //SET_LVDATA_FROM_LINUX;  // set to 1
            CLR_LVDATA_FROM_LINUX;  // set to 0
            //SET_LVDATA_TO_LINUX;  // set to 1
            //CLR_LVDATA_TO_LINUX;  // set to 0

            int strsize = sprintf(TXstring, "Mystring: %s, Count: %d", mystring, countvar);
            ...
            if (GET_LVDATA_TO_LINUX == 1) {
                ...
                ptrshrdmem->DSPSend_size = strsize;
