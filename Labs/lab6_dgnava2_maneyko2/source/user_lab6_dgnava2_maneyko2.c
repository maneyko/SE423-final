...
/*
 * Notes:
 *   After building -> lab6_dgnava2_maneyko2Project\Debug\lab6_dgnava2_maneyko2.hex
 *     - Copy that to Linux (root dir)
 *
 *
 */

int ss = 0;  // switchstate
    ...
    while(1) {
        ...
        if ((tskcount % 6) == 0) {
            if (GET_LVDATA_TO_LINUX) {
                ...
                // Default
               //ptrshrdmem->DSPSend_size = sprintf(toLinuxstring,"1.0 2.0 3.0 4.0");
...
//
//    if (min_front < 500) {
//        vref = 0;  // Stop the robot -> wall is close => begin right wall follow
//        turn = 0;
//
//
//    }
...
//         Checks for missing front wall AND rear right before right turn
    ...
    case 0:  // Print LADAR distances
        ...
        if ((timecount % 100) == 0) {
            LCDPrintfLine(1, "L172:%.1f",
                          LADARdistance[172]);
            LCDPrintfLine(2, "L113:%.1f L54:%.1f",
                          LADARdistance[113], LADARdistance[54]);
        }
        break;
    case 1:  // ADC IR distances

        if ((timecount % 100) == 0) {
            LCDPrintfLine(1, "IR1:%.1f",
                          4095-adcA4);
            LCDPrintfLine(2, "IR2:%.1f", 4095-adcA5);
        }
        break;
    case 2:
        if ((timecount % 100) == 0) {
    ...
    case 3:  // Print Gyro values in radians
        if ((timecount % 100) == 0) {
            LCDPrintfLine(1, "Gyro1:%.1f", (adcA3-1685.36) * 2*PI/360);
            LCDPrintfLine(2, "Gyro2:%.1f", (adcA2-1685.36) * 2*PI/360);
        }
        break;

    case 4:
        if ((timecount % 100) == 0) {
            LCDPrintfLine(1, "Ultra1:%.1f", adcB4);
            LCDPrintfLine(2, "Ultra2:%.1f", adcB5);
        }
        break;

    case 5:
        if ((timecount % 100) == 0) {
            LCDPrintfLine(1, "Vref:%.5f", vref);
            LCDPrintfLine(2, "turn:%.5f", turn);
        }
        break;
    ...
    case 6:
        if ((timecount % 100) == 0) {
            LCDPrintfLine(1, "xA2:%.2f,yA2:%.2f",
                           x_curA2, y_curA2);
            LCDPrintfLine(2, "theta_A2:%.2f,y_curA3:%.2f",
                          theta_A2, y_curA3);

             //turn_command_saturation = 1.0;
        }
