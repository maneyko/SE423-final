...
// ===== STUDENT VARIABLES =====
	...
	float left_turn_Start_threshold = 600.0;
	float left_turn_Stop_threshold = 700.0;
	float Kp_right_wall = -0.001;
...
// ===== END STUDENT VARIABLES =====
	...
	DSPFloats[0] = 1.0;     // vref
	DSPFloats[1] = 0.0;     // turn
	DSPFloats[2] = 400.0;   // ref_right_wall
	DSPFloats[3] = 600.0;   // left_turn_Start_threshold
	DSPFloats[4] = 700.0;   // left_turn_Stop_threshold
	DSPFloats[5] = -0.001;  // Kp_right_wall
	DSPFloats[6] = -0.001;  // Kp_front_wall
	DSPFloats[7] = 0.5;     // front_turn_velocity
	DSPFloats[8] = 2.0;     // turn_command_saturation
	DSPFloats[9] = 0.0;
	DSPFloats[10] = 0.0;
	DSPFloats[11] = 0.0;
	DSPFloats[12] = 0.0;
	...
	while (!inputdone) {
		...
		printf("d - enter ref_right_wall\n");
		printf("f - enter left_turn_Start_threshold\n");
		printf("g - enter left_turn_Stop_threshold\n");
		printf("h - enter Kp_right_wall\n");
		printf("j - enter Kp_front_wall\n");
		printf("k - enter front_turn_velocity\n");
		printf("l - enter turn_command_saturation\n");
		...
		case 's':
			...
			case 'f':
			// request new Velocity reference point from user and send it to DSP
			printf("Enter left_turn_Start_threshold\n");
			fgets(buffer,190,stdin); 
			buffer[strlen(buffer)-1] = '\0';  // get ride of '\n' in returned string
			if (buffer[0] != '\0') {
				if (sscanf(buffer,"%f",&tempfloat) != 0) {  // check that it was a number entered
			...
			case 'g':
			// request new Velocity reference point from user and send it to DSP
			printf("Enter left_turn_Stop_threshold\n");
			fgets(buffer,190,stdin); 
			buffer[strlen(buffer)-1] = '\0';  // get ride of '\n' in returned string
			if (buffer[0] != '\0') {
				if (sscanf(buffer,"%f",&tempfloat) != 0) {  // check that it was a number entered
			...
			case 'k':
			// request new Velocity reference point from user and send it to DSP
			printf("Enter front_turn_velocity\n");
			fgets(buffer,190,stdin); 
			buffer[strlen(buffer)-1] = '\0';  // get ride of '\n' in returned string
			if (buffer[0] != '\0') {
				if (sscanf(buffer,"%f",&tempfloat) != 0) {  // check that it was a number entered
			...
			case 'l':
			// request new Velocity reference point from user and send it to DSP
			printf("Enter turn_command_saturation\n");
			fgets(buffer,190,stdin); 
			buffer[strlen(buffer)-1] = '\0';  // get ride of '\n' in returned string
			if (buffer[0] != '\0') {
				if (sscanf(buffer,"%f",&tempfloat) != 0) {  // check that it was a number entered
			...
			printf("\n\nref_right_wall: %.4f\n", ref_right_wall);
			printf("left_turn_Start_threshold: %.4f\n", left_turn_Start_threshold);
			printf("left_turn_Stop_threshold: %.4f\n", left_turn_Stop_threshold);
			printf("Kp_right_wall: %.4f\n", Kp_right_wall);
			printf("Kp_front_wall: %.4f\n", Kp_front_wall);
			printf("front_turn_velocity: %.4f\n", front_turn_velocity);
			printf("turn_command_saturation: %.4f\n", turn_command_saturation);
			break;
